...
for obs_cov in [0.2, 0.4, 0.8, 1.6]:
    ...
    for file in os.listdir(dist_folder):
        ...
        if 'obs_cov_{}'.format(obs_cov) in file:
            current.append(file)
        else:
            rest.append(file)
    ...
    sns.lineplot(ax=ax, data=data_c, x='time', y='sinkhorn_div', ci='sd', label='obs cov={}'.format(obs_cov))
    ...
    sns.lineplot(ax=ax_all, data=data_c, x='time', y='sinkhorn_div', ci='sd', label='obs cov={}'.format(obs_cov))

fig_all.savefig('plots/obs_gap_{}_obs_cov_{}.png'.format(obs_gap, 'all'))